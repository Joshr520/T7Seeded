detour zm_ai_wasp<scripts\zm\_zm_ai_wasp.gsc>::zm_zod_special_wasp_spawn(n_to_spawn = 1, spawn_point, n_radius = 32, n_half_height = 32, b_non_round, spawn_fx = 1, b_return_ai = 0, spawner_override = undefined)
{
	return zm_zod_special_wasp_spawn(n_to_spawn, spawn_point, n_radius, n_half_height, b_non_round, spawn_fx, b_return_ai, spawner_override);
}

zm_zod_special_wasp_spawn(n_to_spawn = 1, spawn_point, n_radius = 32, n_half_height = 32, b_non_round, spawn_fx = 1, b_return_ai = 0, spawner_override = undefined)
{
	...
	while (count < n_to_spawn) {
		players = GetPlayers();
		favorite_enemy = [[@zm_ai_wasp<scripts\zm\_zm_ai_wasp.gsc>::get_favorite_enemy]]();
		...
		if (IsDefined(ai)) {
			...
			level thread [[@zm_ai_wasp<scripts\zm\_zm_ai_wasp.gsc>::wasp_spawn_init]](ai, v_spawn_origin, spawn_fx);
			count++;
		}
...
detour zm_ai_wasp<scripts\zm\_zm_ai_wasp.gsc>::wasp_spawn_logic(favorite_enemy)
{
	return zm_zod_wasp_spawn_logic(favorite_enemy);
}

zm_zod_wasp_spawn_logic(favorite_enemy)
{
	if (!GetDvarInt("zm_wasp_open_spawning", 0)) {
		wasp_locs = level.zm_loc_types["wasp_location"];
		if (wasp_locs.size == 0) {
			[[@zm_ai_wasp<scripts\zm\_zm_ai_wasp.gsc>::create_global_wasp_spawn_locations_list]]();
			return [[@zm_ai_wasp<scripts\zm\_zm_ai_wasp.gsc>::wasp_find_closest_in_global_pool]](favorite_enemy);
		}
	...
	queryresult = PositionQuery_Source_Navigation(favorite_enemy.origin + (0, 0, SRandomIntRange("zm_ai_wasp_spawn_logic", 40, 100)), 300, spawn_dist_max, 10, 10, "navvolume_small");
	a_points = SArrayRandomize(queryresult.data, "zm_ai_wasp_spawn_logic");
...
detour zm_ai_wasp<scripts\zm\_zm_ai_wasp.gsc>::wasp_round_tracker()
{
	...
	for (;;) {
		...
		if (level.round_number == level.next_wasp_round) {
			...
			old_spawn_func = level.round_spawn_func;
			old_wait_func = level.round_wait_func;
			[[@zm_ai_wasp<scripts\zm\_zm_ai_wasp.gsc>::wasp_round_start]]();
			level.round_spawn_func = @zm_ai_wasp<scripts\zm\_zm_ai_wasp.gsc>::wasp_round_spawning;
			level.round_wait_func = @zm_ai_wasp<scripts\zm\_zm_ai_wasp.gsc>::wasp_round_wait_func;
			...
			[[@zm_ai_wasp<scripts\zm\_zm_ai_wasp.gsc>::wasp_round_stop]]();
...
detour zm_ai_wasp<scripts\zm\_zm_ai_wasp.gsc>::spawn_wasp()
{
	b_swarm_spawned = 0;
	while (!b_swarm_spawned) {
		while (![[@zm_ai_wasp<scripts\zm\_zm_ai_wasp.gsc>::ready_to_spawn_wasp]]()) {
			wait 1;
		}
		...
		if (a_spawn_origins.size >= 1) {
			...
			while (n_spawn < 1 && level.zombie_total > 0) {
				for (i = a_spawn_origins.size - 1; i >= 0; i--) {
					...
					if (IsDefined(ai)) {
						ai [[@parasite<scripts\shared\vehicles\_parasite.gsc>::set_parasite_enemy]](favorite_enemy);
						level thread [[@zm_ai_wasp<scripts\zm\_zm_ai_wasp.gsc>::wasp_spawn_init]](ai, v_origin);
